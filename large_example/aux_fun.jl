...
function load_network_data_new(caseID)
    data_net = PowerModels.parse_file(caseID)
    # Network size
    G = length(data_net["gen"])
    N = length(data_net["bus"])
    ...
    # Initialize data center data 
    dc̅ = ones(DC).*100
    dc̲ = ones(DC).*0
    dc = (dc̅ .+ dc̲) ./ 2
    Δ = sum(dc)
    M_dc = zeros(N,DC)
    # select buses to place data centers 
    data_centers = [7,9,14,35,41,45,53,75,97,108]
    for i = 1:DC
        M_dc[data_centers[i],i] = 1;
    end
    ...
    :d => d, :M_d => M_d,
    # load data_centers
    :dc̅ => dc̅, :dc̲ => dc̲, :dc => dc, :Δ => Δ, :M_dc => M_dc,
    # generation data
    :p̅ => p̅, :p̲ => p̲, :M_p => M_p,
    :c1 => c1, :c2 => c2, :c0=> c0,
    # graph data
    :N => N, :E => E, :G => G, :D => D, :ref => ref, :DC => DC
    )
    return net
end
...
function solve_OPF(net)
    ...
    # OPF equations
    @constraint(model, μ, ones(net[:N])'*(net[:M_p]*p .- net[:M_d]*net[:d] .- net[:M_dc]*net[:dc]) .== 0)
    @constraint(model, μ̅, net[:f̅] .>=  net[:T]*(net[:M_p]*p .- net[:M_d]*net[:d] .- net[:M_dc]*net[:dc]))
    @constraint(model, μ̲, net[:f̅] .>= -net[:T]*(net[:M_p]*p .- net[:M_d]*net[:d] .- net[:M_dc]*net[:dc]))
    ...
    sol = Dict(:status => termination_status(model),
                ...
                :λ =>  net[:M_dc]'*π.*net[:dc],#net[:M_d]'*π.*net[:d],
                :π => π)
    return sol
end
